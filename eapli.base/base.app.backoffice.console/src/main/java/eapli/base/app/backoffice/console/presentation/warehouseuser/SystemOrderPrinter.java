...
public class SystemOrderPrinter implements Visitor<Orders> {
    ...
    public void visit(Orders visitee) {
        System.out.printf("%-10s%-20s%-30s%-40s%-50s%-60s%-60s", visitee.getId(), visitee.generateTime(), visitee.getAddress(), visitee.getShipmentMethod(),visitee.getPayment_method(),visitee.getShopping_cart(),visitee.getOrderLevel());
    }
}
